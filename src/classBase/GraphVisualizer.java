...
public class GraphVisualizer extends Frame {
	 ...
	 * @param order A 2d-array of strings, where the indices, which contain the vertex's name, (times dx for the x-coordinate and times dy for the y-coordinate) resemble the position of the vertex
	 * @param dx The distance between the vertices in x direction
	 * @param dy The distance between the vertices in y direction
	 * @param drawVertexCircle True -> Vertex = Name + Circle; False -> Vertex = Name
	 * 
	 * @author Croyd
	 * */
	public GraphVisualizer(String frameTitle, int width, int height, Graph graph, String[][] order, int dx, int dy, boolean drawVertexCircle) {
		super(frameTitle);
		this.graph = graph;
	...
	/**
	 * A class to visualize the given graph in a frame. 
	 * The parameter 'positions' contains the x- and y-coordinates of the vertex with the index i in the list that is returned by graph.getVertices() -> positions[i][0] to get x and positions[i][1] to get y of the vertex with the index i
	 * 
	 * 
	 * @param frameTitle The title of the frame.
	 * @param width The width of the frame.
	 * @param height The height of the frame.
	 * @param graph The graph that should be displayed.
	 * @param positions An 2d-array that contains the x- and y-coordinate of the vertices. Dimension: [...][2] !
	 * @param drawVertexCircle True -> Vertex = Name + Circle; False -> Vertex = Name
	 * 
	 * @author Croyd
	 * */
	public GraphVisualizer(String frameTitle, int width, int height, Graph graph, int[][] positions, boolean drawVertexCircle) {
		super(frameTitle);
		this.graph = graph;
	...
	private void zeichne(String[][] order, int dx, int dy, boolean drawVertexCircle) {
        ...
        //'simulating' the calculating of the vertices' position to get their coordinates so the lines can be drawn properly
        vertices.toFirst();
        while (vertices.hasAccess()) {
        	...
        	for (int i = 0; i < order.length; i++) {
        		for (int j = 0; j < order[0].length; j++) {
        			if (vertices.getContent().getID().equals(order[i][j])) {	//saving the calculated position in vertexPosition
        ...
        while (vertices.hasAccess()) {
        	...
        	while (neighbors.hasAccess()) {
        		...
        		g.drawLine(vertexPosition[id1][0] + 10, vertexPosition[id1][1] + 10, vertexPosition[id2][0] + 10, vertexPosition[id2][1] + 10);
	...
	private void zeichne(int[][] positions, boolean drawVertexCircle) {
        ...
        while (vertices.hasAccess()) {
        	...
        	while (neighbors.hasAccess()) {
        		...
        		g.setColor(new Color(0, 0, 0));
        		g.drawLine(positions[id1][0] + 10, positions[id1][1] + 10, positions[id2][0] + 10, positions[id2][1] + 10);
